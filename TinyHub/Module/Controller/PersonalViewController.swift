...
class PersonalViewController: ListViewController {
    ...
    func tapUser(_: Void? = nil) {
        if self.reactor?.currentState.user?.isValid ?? false {
            self.navigator.jump(Router.shared.urlString(host: .profile))
            return
        }
    ...
    func tapRepositories(_: Void? = nil) {
        // YJX_TODO 测试位置
        // self.navigator.forward("https://github.com/Tencent/QMUI_iOS")
        // self.navigator.forward("https://profile-summary-for-github.com/user/tospery")
//        let string = "\(UIApplication.shared.baseWebUrl)/\(Author.username)"
//        log(string)
//        self.navigator.forward(string)
//        
//        var myUser1: Any? = nil
//        let myUser = User.init(JSONString: "{\"id\": 101,\"username\": \"tospery\"}")
//        myUser1 = myUser
//        let bbb = String.init(describing: myUser1!)
//        log("")
